    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        <li class="breadcrumb-item active">
          ...
          <a href=" {{ route('officer.thongtinthitruong.index') }} "><i class="breadcrumb-icon fa fa-angle-left mr-2"></i>Thông tin thị trường</a>
        </li>
      </ol>
    </nav>
    <div class="d-md-flex align-items-md-start">
      <h1 class="page-title mr-sm-auto"> Thông tin thị trường <small> Tạo mới </small> </h1>
    </div>
  </header>
  <div class="page-section">
    <div class="card card-fluid">
      <div class="card-body">
        ...
        <form action=" {{ route('officer.thongtinthitruong.store') }} " method="post">
          @csrf()
          <fieldset>
              ...
              <select id="nongsan_id" class="form-control custom-select @error('nongsan_id') is-invalid @enderror" name="nongsan_id"  placeholder="Chọn nông sản" autofocus >
                  <option value="">-- Choose --</option>
                      @foreach($dsNongSan as $value)
                            <option value= "{{ $value->id }}" {{$value->id == old('nongsan_id') ? ' selected' : ''}}> {{$value->tennongsan}} </option>
                      @endforeach
              </select>
            ...
            <div class="form-group">
              ...
              <input type="date" class="form-control @error('ngaycapnhat') is-invalid @enderror" id="ngaycapnhat" name="ngaycapnhat" value="{{old('ngaycapnhat')}}" placeholder="Ngày cập nhật" >
            ...
            <div class="form-group">
              ...
              <input type="number" min="0" step="0.01"  class="form-control @error('dongia') is-invalid @enderror" id="dongia" name="dongia" value="{{old('dongia')}}" placeholder="0.00"  >
            ...
            <div class="form-group">
              ...
              <textarea class="form-control @error('chinhsachkhuyenmai') is-invalid @enderror" id="chinhsachkhuyenmai" name="chinhsachkhuyenmai" rows="3" placeholder="Chính sách khuyến mãi" autofocus>{{old('chinhsachkhuyenmai')}}</textarea>
