    ...
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        <li class="breadcrumb-item active">
          ...
          <a href=" {{ route('officer.nongsan.index') }} "><i class="breadcrumb-icon fa fa-angle-left mr-2"></i>Nông sản</a>
        </li>
      </ol>
    </nav>
  ...
  <div class="page-section">
    <div class="card card-fluid">
      <div class="card-body">
        ...
        <form action=" {{ route('officer.nongsan.update') }} " method="post">
          @csrf()
          @method('PUT')
          <input type="hidden" name="nongsanid" value=" {{ $nongSan->id }} ">

          <fieldset>
            <div class="form-group">
              <label for="tennongsan">Tên nông sản <abbr title="Required">*</abbr></label>
              <input type="text" class="form-control @error('tennongsan') is-invalid @enderror" id="tennongsan" name="tennongsan" value="{{old('tennongsan',  $nongSan->tennongsan)}}" placeholder="Tên loại nông sản" autofocus>
              ...
              <select id="loainongsan_id" class="form-control custom-select @error('loainongsan_id') is-invalid @enderror" name="loainongsan_id"  placeholder="Chọn loại nông sản" >
                  <option value="">-- Choose --</option>
                      @foreach($dsLoaiNongSan as $value)
                        <option value="{{ $value->id }}" {{ ($nongSan->loainongsan_id == $value->id) ? 'selected' : '' }}>{{ $value->tenloainongsan }}</option>
                      @endforeach
              </select>
            ...
            <div class="form-group">
              ...
              <textarea class="form-control @error('mota') is-invalid @enderror" id="mota" name="mota" rows="3" placeholder="Mô tả" autofocus>{{old('mota', $nongSan->mota)}}</textarea>
