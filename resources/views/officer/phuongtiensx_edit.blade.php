...
<div class="page-inner">
  <header class="page-title-bar">
    ...
    <div class="d-md-flex align-items-md-start">
      <h1 class="page-title mr-sm-auto"> Phương tiện sản xuất <small> Chỉnh sửa </small> </h1>
    </div>
  </header>
  <div class="page-section">
    <div class="card card-fluid">
      <div class="card-body">
        ...
        <form action=" {{ route('officer.phuongtiensx.update') }} " method="post">
          @csrf()
          @method('PUT')
          <input type="hidden" name="phuongtiensxid" value=" {{ $phuongTienSX->id }} ">

          <fieldset>
            <div class="form-group">
              <label for="masophuongtien">Mã số <abbr title="Required">*</abbr></label>
              <input type="text" class="form-control @error('masophuongtien') is-invalid @enderror" id="masophuongtien" name="masophuongtien" value="{{ old('masophuongtien',  $phuongTienSX->masophuongtien) }}" placeholder="Mã số phương tiện" autofocus>
              ...
              <select id="hoptacxa_id" class="form-control custom-select @error('hoptacxa_id') is-invalid @enderror" name="hoptacxa_id"  placeholder="Chọn hợp tác xã" >
                  <option value="">-- Choose --</option>
                      @foreach($dsHopTacXa as $value)
                        <option value="{{ $value->id }}" {{ ($phuongTienSX->hoptacxa_id == $value->id) ? 'selected' : '' }}>{{ $value->tenhtx }}</option>
                      @endforeach
              </select>
              ...
              <select id="loaiphuongtien_id" class="form-control custom-select @error('loaiphuongtien_id') is-invalid @enderror" name="loaiphuongtien_id"  placeholder="Chọn loại phương tiện" >
                  <option value="">-- Choose --</option>
                      @foreach($dsLoaiPhuongTien as $value)
                         <option value="{{ $value->id }}" {{ ($phuongTienSX->loaiphuongtien_id == $value->id) ? 'selected' : '' }}>{{ $value->tenloaiphuongtien }}</option>
                      @endforeach
              </select>
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control @error('tenphuongtien') is-invalid @enderror" id="tenphuongtien" name="tenphuongtien" value="{{ old('tenphuongtien',  $phuongTienSX->tenphuongtien) }}" placeholder="Tên phương tiện phương tiện" >
            ...
            <div class="form-group">
              ...
              <input type="date" class="form-control @error('ngaymua') is-invalid @enderror" id="ngaymua" name="ngaymua" value="{{ old('ngaymua',  $phuongTienSX->ngaymua) }}" placeholder="Ngày mua" >
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control @error('nhasanxuat') is-invalid @enderror" id="nhasanxuat" name="nhasanxuat" value="{{ old('nhasanxuat',  $phuongTienSX->nhasanxuat) }}" placeholder="Nhà sản xuất" >
              @error('nhasanxuat')  <div class="invalid-feedback"> <i class="fa fa-exclamation-circle fa-fw"></i> {{ $message }} </div>  @enderror
            </div>
            
            <div class="form-group">
              <label for="namsanxuat">Năm sản xuất </label>
              <input type="text" class="form-control @error('namsanxuat') is-invalid @enderror" id="namsanxuat" name="namsanxuat" value="{{ old('namsanxuat',  $phuongTienSX->namsanxuat) }}" placeholder="Năm sản xuất" >
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control @error('nuocsanxuat') is-invalid @enderror" id="nuocsanxuat" name="nuocsanxuat" value="{{ old('nuocsanxuat',  $phuongTienSX->nuocsanxuat) }}" placeholder="Nước sản xuất" >
              @error('nuocsanxuat')  <div class="invalid-feedback"> <i class="fa fa-exclamation-circle fa-fw"></i> {{ $message }} </div>  @enderror
            </div>
            
            <div class="form-group">
              <label for="dientich">Diện tích </label>
              <input type="text" class="form-control @error('dientich') is-invalid @enderror" id="dientich" name="dientich" value="{{ old('dientich',  $phuongTienSX->dientich) }}" placeholder="Diện tích" >
              @error('dientich')  <div class="invalid-feedback"> <i class="fa fa-exclamation-circle fa-fw"></i> {{ $message }} </div>  @enderror
            </div>
            
            <div class="form-group">
              <label for="vitri">Vị trí </label>
              <input type="text" class="form-control @error('vitri') is-invalid @enderror" id="vitri" name="vitri" value="{{ old('vitri',  $phuongTienSX->vitri) }}" placeholder="Vị trí" >
              @error('vitri')  <div class="invalid-feedback"> <i class="fa fa-exclamation-circle fa-fw"></i> {{ $message }} </div>  @enderror
            </div>
            
            <div class="form-group">
              <label for="mota">Mô tả </label>
              <textarea class="form-control @error('mota') is-invalid @enderror" id="mota" name="mota" rows="3" placeholder="Mô tả" >{{ old('mota',  $phuongTienSX->mota) }}</textarea>
              @error('mota')  <div class="invalid-feedback"> <i class="fa fa-exclamation-circle fa-fw"></i> {{ $message }} </div>  @enderror
            </div>

            <div class="form-group">
              <label for="trangthai">Trạng thái </label>
              <input type="text" class="form-control @error('trangthai') is-invalid @enderror" id="trangthai" name="trangthai" value="{{ old('trangthai',  $phuongTienSX->trangthai) }}" placeholder="Trạng thái" >
