...
<body>
	<main class="auth">
		<form action="{{ route('password.email') }}" method="post" class="auth-form auth-form-reflow">
			@csrf
			...
			<div class="form-group mb-4">
				...
				<input type="email" id="email" name="email" value="{{ old('email') }}" class="form-control form-control-lg @error('email') is-invalid @enderror" required autofocus />
				@error('email')
					<span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
				@enderror
			</div>
			<div class="d-block d-md-inline-block mb-2">
				<button class="btn btn-lg btn-block btn-primary" type="submit">Gởi email khôi phục</button>
			</div>
		</form>
